...
def afterlogin_view(request):
    ...
    # Check if the user is authenticated with the standard login
    if is_student(user):
        return redirect('student/student-dashboard')

    # Check if the user is authenticated with GitHub
    elif models.Student.objects.filter(user__first_name=user.username).exists():
            return redirect('student/student-dashboard')

    # Check if the user is authenticated with the standard login and is a teacher
    elif is_teacher(user):
        accountapproval = TMODEL.Teacher.objects.all().filter(user_id=user.id, status=True)
        if accountapproval:
            return redirect('teacher/teacher-dashboard')
        else:
            return render(request, 'teacher/teacher_wait_for_approval.html')

    # If the user is not authenticated with any method, redirect to the admin dashboard
    else:
        return redirect('admin-dashboard')
...
def contactus_view(request):
    ...
    if request.method == 'POST':
        ...
        if sub.is_valid():
            ...
            # send_mail(str(name)+' || '+str(email),message,settings.EMAIL_HOST_USER, ['raltair2002@gmail.com'], fail_silently = False)
            send_mail(str(name), message,settings.EMAIL_HOST_USER, ['raltair2002@gmail.com'], fail_silently = False)
